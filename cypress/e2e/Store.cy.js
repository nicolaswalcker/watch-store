...
context('Store', () => {
  ...
  context('Store > Search for products', () => {
    ...
    it('should return 1 product when "Relófio bonito" is used as search term', () => {
  ...
  context('Store > Product List', () => {
    it('should display "0 Pproducts when no product is returned"', () => {
      cy.visit('/');
      gid('product-card').should('have.length', 0);
      g('body').contains('0 Products');
    });
    ...
    it('should display "1 Products when 10 products are returned"', () => {
      server.createList('product', 10);
  ...
  context('Store > Shopping Cart', () => {
    ...
    it('should toggle shopping cart visibility when button is clicked', () => {
      gid('toggle-button').as('toggleButton'); // Cypress trabalha de forma assincrona
      g('@toggleButton').click(); // por isso é necessário usar o .as() para referenciar o elemento
    ...
    it('should not display "Clear cart" button when cart is empty', () => {
      gid('toggle-button').as('toggleButton');
      g('@toggleButton').click();
      gid('clear-cart-button').should('not.exist'); // Verifica se o elemento existe na DOM
    }); // Caso tente utilizar o not.be.visible, o teste falhará, pois o elemento não existe na DOM no caso do uso do v-if
